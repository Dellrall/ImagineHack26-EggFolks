...
type transitLineInfo struct {
	Name      string `json:"name"`
	ShortName string `json:"shortName"`
}

type transitDetailsInfo struct {
	TransitLine transitLineInfo `json:"transitLine"`
}
...
type stepInfo struct {
	...
	TransitDetails *transitDetailsInfo `json:"transitDetails"`
}
...
func FetchMultiModeRoutes(ctx context.Context, origin, destination, apiKey string) ([]GoogleRoute, error) {
	...
	if len(transitResp.Routes) > 0 {
		...
		if len(route.Legs) > 0 {
			...
			// Attempt to extract LRT/MRT/Bus names from steps
			var segments []string
			for _, step := range leg.Steps {
				if step.TravelMode == "TRANSIT" && step.TransitDetails != nil {
					lineName := step.TransitDetails.TransitLine.Name
					if lineName == "" {
						lineName = step.TransitDetails.TransitLine.ShortName
					}
					if lineName != "" {
						segments = append(segments, lineName)
					}
				}
			}
			if len(segments) == 0 {
				for _, step := range leg.Steps {
					if step.TravelMode == "TRANSIT" && step.NavigationInstruction != nil {
...
func computeGoogleRoute(ctx context.Context, origin, destination, travelMode, apiKey string) (*routesAPIResponse, error) {
	...
	if travelMode == "DRIVE" {
		...
		reqBody.DepartureTime = time.Now().Add(5 * time.Minute).UTC().Format(time.RFC3339)
	}
	...
	req.Header.Set("X-Goog-FieldMask", "routes.duration,routes.distanceMeters,routes.legs.localizedValues,routes.legs.steps.transitDetails,routes.legs.steps.navigationInstruction,routes.legs.steps.travelMode")
